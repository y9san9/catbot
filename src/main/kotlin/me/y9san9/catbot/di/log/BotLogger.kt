    ...
    override fun processEvent(event: LogEvent) {
        val string = when (event) {
            ...
            is LogEvent.ChatMemberJoined.New ->
                "New chat member ${event.member.title} (${event.member.link}) joined in chat " +
                        "${event.member.chat.title} (${event.member.chat.link})"
            is LogEvent.ChatMemberJoined.Old ->
                "Old chat member ${event.member.title} (${event.member.link}) joined again in chat " +
                        "${event.member.chat.title} (${event.member.chat.link})"
            ...
            is LogEvent.CouldNotSentTheGif -> "Could not send the gif to chat ${event.chat.title} (${event.chat.link}."+
